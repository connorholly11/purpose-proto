const { queryDocuments } = require('../lib/services/pinecone');
...
describe('RAG System Verification', () => {
  test('Test embedding generation, Pinecone query, and RAG logging', async () => {
    ...
    for (const query of testQueries) {
      ...
      // Use the queryDocuments function to perform RAG
      ...
      console.log(`Query execution time: ${result.operationTime}ms`);
      ...
      if (result.matches.length > 0) {
        console.log('Top match score:', result.matches[0].score);
        // First 50 chars of matched content
        console.log('Sample content:', result.matches[0].text.substring(0, 50) + '...');
      }
      ...
      expect(operation.source).toBe('test');
